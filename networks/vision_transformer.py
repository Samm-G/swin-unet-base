...
class SwinUnet(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.swin_unet = SwinTransformerSys(img_size=config.DATA.IMG_SIZE,
                                patch_size=int(config.MODEL.SWIN.PATCH_SIZE),
                                ...
                                qkv_bias=bool(config.MODEL.SWIN.QKV_BIAS),
                                qk_scale=config.MODEL.SWIN.QK_SCALE,
                                drop_rate=float(config.MODEL.DROP_RATE),
                                drop_path_rate=float(config.MODEL.DROP_PATH_RATE),
                                ape=bool(config.MODEL.SWIN.APE),
                                patch_norm=bool(config.MODEL.SWIN.PATCH_NORM),
